...
received array"]` â or the reverse mistake with `score` questions.
...
```jsonc
// â choice â criteria is a RECORD: {option: "what it means"}
{
...
// â choice with an ARRAY fails: ["billing", "technical"]

// â score â criteria is an ARRAY of ordered rungs (min 2), lowest first
{
  "questions": {
    "urgency": {
      "type": "score",
...
The 400 error is accurate but only names the failing question field â if you
...
for `noul` â verify against the spec, not the doc.

## Verification and sources

- **live-test** 2026-09-21: array-criteria `choice` â 400 `expected record,
  received array`; dict-criteria `choice` + array `score` â 200 OK.
- **source-review** 2026-09-21: OpenAPI spec served by the gateway
  (`gen.pollinations.ai/openapi.json`), `/alpha/decisions` request schema â
